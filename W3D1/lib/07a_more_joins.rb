...
def song_title_counts
  ...
  execute(<<-SQL)
    SELECT
      tracks.song, COUNT(DISTINCT albums.asin) AS num_albums
    FROM
      tracks
    JOIN
      albums ON tracks.album = albums.asin
...
def top_track_counts
  ...
  execute(<<-SQL)
    SELECT
      albums.title, COUNT(*) AS track_count
    FROM
      albums
    JOIN
      tracks ON albums.asin = tracks.album
...
def expensive_tastes
  ...
  execute(<<-SQL)
    SELECT
      styles.style,
      (SUM(album_counts.price) /
        SUM(album_counts.num_tracks)) AS value
    FROM (
      SELECT
        albums.asin, albums.title, albums.price, COUNT(*) AS num_tracks
      FROM
        albums
      JOIN
        tracks ON albums.asin = tracks.album
      ...
      GROUP BY
        albums.asin, albums.title, albums.price
      ) AS album_counts
    JOIN
      styles ON album_counts.asin = styles.album
    GROUP BY
      styles.style
    ORDER BY
      value DESC
